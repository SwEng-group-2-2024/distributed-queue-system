...
app.get("/isEmpty", (req, res) => {
  res.status(200).send(queue.length === 0);
});

// Method to get the next item to be dequeued from the queue
app.get("/nextOut", (req, res) => {
  if (queue.length > 0) {
    res.status(200).send(queue[0]);
  } else {
